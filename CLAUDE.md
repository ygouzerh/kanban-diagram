...
- **`src/components/GraphCanvas.tsx`** (right pane): joins `cards → elements` (via a `cardByElement` map) to assign each node/edge its `status`-derived color before rendering. Custom node type `status` = `StatusNode`, custom edge type `floating` = `FloatingEdge`.
- **`src/components/StatusNode.tsx`**: custom node with draw.io-style handles (4 sides × 3 points), shows the card description as a hover tooltip.
- **`src/components/FloatingEdge.tsx` + `floating.ts`**: edges that attach to the nearest node boundary (geometry in `floating.ts`).
